...
def update_permission():
    if request.method == 'POST':
        steam_id = request.form['steam_id']
        permission_group = request.form['permission_group']
        
        try:
            # Connect to the database for updating the permission group
            connection = get_atlasdb_connection()
            with connection.cursor() as cursor:
                # Update the permission group for the given Steam ID
                cursor.execute("""
                    UPDATE players
                    SET PermissionGroups = %s
                    WHERE SteamId = %s
                """, (permission_group, steam_id))
                connection.commit()
            
            flash('Permission updated successfully!', 'success')
        except Exception as e:
            flash(f'Error updating permission: {e}', 'danger')
        finally:
            connection.close()
        
        # Redirect back to the permissions page after updating
